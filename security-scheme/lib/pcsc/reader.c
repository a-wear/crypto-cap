...
int32_t sc_get_card_connection(reader_t *reader)
{
    ...
    // Connect to the card
    rv = SCardConnect(reader->hContext, &mszReaders[0], SCARD_SHARE_SHARED, (unsigned) SCARD_PROTOCOL_T0 | (unsigned) SCARD_PROTOCOL_T1, &reader->hCard, &reader->dwActiveProtocol);
    if (rv != SCARD_S_SUCCESS)
    {
        SCardReleaseContext(reader->hContext);
        return rv;
...
int32_t sc_transmit_data(reader_t reader, const uint8_t *pbSendBuffer, uint32_t dwSendLength, uint8_t *pbRecvBuffer, uint32_t *dwRecvLength, double *elapsed_time)
{
    ...
    switch (reader.dwActiveProtocol)
    {
    ...
    pioRecvPci.dwProtocol = reader.dwActiveProtocol;
