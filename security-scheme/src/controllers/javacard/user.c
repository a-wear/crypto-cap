...
int ue_exchange_identifier_nonce(reader_t reader, user_identifier_t *identifier, verifier_identifier_t ve_identifier, user_nonce_t *nonce, verifier_nonce_t ve_nonce)
{
    ...
    le = USER_MAX_ID_LENGTH + NONCE_LENGTH;

    dwSendLength = sizeof(pbSendBuffer);
    r = apdu_build_command(CASE2S, CLA_APPLICATION, INS_GET_USER_IDENTIFIER_NONCE, 0x00, 0x00, 0, NULL, le, pbSendBuffer, &dwSendLength);
    if (r < 0)
    {
        return -1;
    }
    ...
    // user nonce
    memcpy(nonce->nonce, &pbRecvBuffer[USER_MAX_ID_LENGTH], NONCE_LENGTH);

    /// exchange verifier information
    lc = 0;
    ...
    dwSendLength = sizeof(pbSendBuffer);
    r = apdu_build_command(CASE3S, CLA_APPLICATION, INS_SET_VERIFIER_IDENTIFIER_NONCE, 0x00, 0x00, lc, data, 0, pbSendBuffer, &dwSendLength);
    if (r < 0)
    {
        return -1;
    }
                    ...
                    user_private_key_t ue_private_key, user_cipher_output_t *ue_cipher_output)
{
    ...
    // expected data length to be received
    le = AES_IV_LENGTH + sizeof(user_cipher_output_t);

    dwSendLength = sizeof(pbSendBuffer);
    r = apdu_build_command(CASE2S, CLA_APPLICATION, INS_COMPUTE_SHOW_STAGE_1, 0x00, 0x00, 0, NULL, le, pbSendBuffer, &dwSendLength);
    if (r < 0)
    {
        return -1;
    }
    ...
    memcpy(parameters->iv_user, pbRecvBuffer, AES_IV_LENGTH);
    // copy encrypted output
    memcpy(ue_cipher_output->cipher_output, &pbRecvBuffer[AES_IV_LENGTH], sizeof(user_cipher_output_t));
                    ...
                    user_keys_t *ue_keys, verifier_cipher_output_t ve_cipher_output)
{
    ...
    // verifier iv
    memcpy(&data[lc], parameters->iv_verifier, AES_IV_LENGTH);
    lc += AES_IV_LENGTH;
    ...
    dwSendLength = sizeof(pbSendBuffer);
    r = apdu_build_command(CASE3S, CLA_APPLICATION, INS_COMPUTE_SHOW_STAGE_2, 0x00, 0x00, lc, data, 0, pbSendBuffer, &dwSendLength);
    if (r < 0)
    {
        return -1;
    }
