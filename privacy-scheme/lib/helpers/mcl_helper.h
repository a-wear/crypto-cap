...
#ifndef __PRIVACY_SCHEME_MCL_HELPER_H_
#define __PRIVACY_SCHEME_MCL_HELPER_H_
...
/**
 * Converts the mclBnFr type into an array of bytes (smartcard fr).
 *
 * @param buffer the buffer where the conversion will be stored
 * @param buffer_length the length of the buffer
 * @param x mclBnFr data
 * @return 0 if success else -1
 */
extern int mcl_Fr_to_smartcard_Fr(void *buffer, size_t buffer_length, mclBnFr x);

/**
 * Converts the mclBnFr type into an array of bytes (smartcard multiplier).
 *
 * @param buffer the buffer where the conversion will be stored
 * @param buffer_length the length of the buffer
 * @param x mclBnFr data
 * @return 0 if success else -1
 */
extern int mcl_Fr_to_smartcard_Multiplier(void *buffer, size_t buffer_length, mclBnFr x);

/**
 * Converts the mclBnG1 type into an array of bytes (smartcard point).
 *
 * @param buffer the buffer where the conversion will be stored
 * @param buffer_length the length of the buffer
 * @param x mclBnG1 data
 * @return 0 if success else -1
 */
extern int mcl_G1_to_smartcard_G1(void *buffer, size_t buffer_length, mclBnG1 x);

#ifdef __cplusplus
}
#endif

#endif /* __PRIVACY_SCHEME_MCL_HELPER_H_ */
